...
// ═══ LOCAL CACHE SYSTEM ═══
// Provides instant data display from localStorage before Firebase loads
const jpCache = {
    save(key, data) {
        try {
            localStorage.setItem('jp_' + key, JSON.stringify({
                data: data,
                ts: Date.now()
            }));
        } catch (e) { /* quota exceeded — ignore */ }
    },
    load(key, maxAgeMs = 24 * 60 * 60 * 1000) {
        try {
            const raw = localStorage.getItem('jp_' + key);
            if (!raw) return null;
            const parsed = JSON.parse(raw);
            // Ignore stale data older than maxAge (default 24h)
            if (Date.now() - parsed.ts > maxAgeMs) {
                localStorage.removeItem('jp_' + key);
                return null;
            }
            return parsed.data;
        } catch (e) { return null; }
    ...
    clear() {
        ['user', 'games', 'slider', 'announcement'].forEach(k => {
            localStorage.removeItem('jp_' + k);
        });
    }
};
...
function showAppShell() {
    console.log('[PERF] showAppShell() called, already shown:', !!window._appShellShown);
    ...
    // ══ INSTANT CACHE: Show cached data immediately while Firebase loads ══
    const cachedUser = jpCache.load('user');
    if (cachedUser) {
        const depositBal = cachedUser.depositBalance || 0;
        const winningBal = cachedUser.winningBalance || 0;
        const totalBal = depositBal + winningBal;
        const setEl = (id, val) => { const el = document.getElementById(id); if (el) el.innerHTML = val; };
        setEl('header-balance', formatCoin(totalBal));
        setEl('profile-fullname', cachedUser.fullName || 'User');
        setEl('profile-balance', formatCoin(totalBal));
        console.log('[CACHE] Displayed cached user data instantly');
    } else {
        // No cache — show loading placeholders
        const setLoading = (id, val) => { const el = document.getElementById(id); if (el) el.innerText = val; };
        setLoading('header-balance', '🪙 ...');
        setLoading('profile-fullname', 'Loading...');
        setLoading('profile-balance', '🪙 ...');
    }

    // ══ GUARD: Only load data ONCE per session ══
    if (window._appShellShown) return;
    window._appShellShown = true;
    console.log('[PERF] First showAppShell — loading data');

    // Start loading content in parallel (non-blocking)
    loadHome();
    // Deferred scripts may not be loaded yet — safe call
    if (typeof loadEsportsGames === 'function') loadEsportsGames();
    else window.addEventListener('load', () => { if (typeof loadEsportsGames === 'function') loadEsportsGames(); }, { once: true });
    if (typeof loadChallenges === 'function') loadChallenges();
    else window.addEventListener('load', () => { if (typeof loadChallenges === 'function') loadChallenges(); }, { once: true });
}
...
function showApp() {
    // showApp is an alias for showAppShell (guarded)
    showAppShell();
}

// Reset app state flags on logout — called from auth.js
function resetAppFlags() {
    window._appShellShown = false;
    window._homeLoaded = false;
    window._dailyRewardChecked = false;
    window._onboardingChecked = false;
    window._sessionInitDone = false;
    window._earnCurrentCategory = null;

    // Reset listener guards added by optimization (esports.js, ludo.js)
    // These must be reset so listeners re-attach on re-login
    if (typeof _esportsGamesListenerAttached !== 'undefined') _esportsGamesListenerAttached = false;
    if (typeof _ludoListenerAttached !== 'undefined') _ludoListenerAttached = false;
...
function updateUIHeader() {
    if (state.userData) {
        // ══ CACHE: Save user data for instant display on next app open ══
        jpCache.save('user', state.userData);
        ...
        if (typeof loadProfileAvatar === 'function') {
            loadProfileAvatar(state.userData.photoURL || null);
        }
