...
async function refundPlayer(playerData, matchId, matchAmount, reason) {
    ...
    let depositRefund = playerData.depositDeducted || 0;
    let winningRefund = playerData.winningDeducted || 0;

    // CRITICAL FALLBACK: If deduction records are missing but matchAmount > 0,
    // refund the full amount to depositBalance to prevent silent money loss.
    // This handles legacy matches or data inconsistencies where depositDeducted/
    // winningDeducted were not stored in the match object.
    if (depositRefund === 0 && winningRefund === 0 && matchAmount > 0) {
        functions.logger.warn(
            `[refundPlayer] Missing deduction records for ${playerUid} in match ${matchId}. ` +
            `Falling back to full refund of ${matchAmount} to depositBalance.`
        );
        depositRefund = matchAmount;
    }
...
async function countUserLudoMatches(uid, statusFilter) {
    ...
    // Also count matches where user is acceptor (indexed query, not full scan)
    const acceptorSnap = await db.ref('ludo_matches')
        .orderByChild('acceptor/uid').equalTo(uid)
        .once('value');
    acceptorSnap.forEach(snap => {
        const m = snap.val();
        if (statusFilter.includes(m.status)) count++;
    });
    return count;
...
async function checkAndCreditReferral(playerUid) {
    try {
        ...
        // Count eSports participations via wallet_transactions (much cheaper than scanning all matches)
        const esportsTxnSnap = await db.ref('wallet_transactions')
            .orderByChild('userId').equalTo(playerUid).once('value');
        if (esportsTxnSnap.exists()) {
            esportsTxnSnap.forEach(child => {
                const t = child.val();
                // Count match entries (JOIN_MATCH or DEBIT with matchId) as completed game participations
                if ((t.type === 'JOIN_MATCH' || t.type === 'DEBIT') && t.matchId) {
                    completedGames++;
                }
            });
        }
