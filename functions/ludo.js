...
exports.submitLudoResult = functions.https.onCall(async (data, context) => {
    const uid = assertAuth(context);
    const { matchId, resultType, screenshotUrl } = data;

    if (!matchId || !resultType) throw new functions.https.HttpsError('invalid-argument', 'matchId and resultType required');
    ...
    if (resultType === 'LOST') {
        ...
        if (!['ROOM_SHARED', 'IN_PROGRESS'].includes(match.status)) {
            throw new functions.https.HttpsError('failed-precondition', 'Cannot submit result in current state');
        }

        const isCreator = match.creator?.uid === uid;
        const isAcceptor = match.acceptor?.uid === uid;
        if (!isCreator && !isAcceptor) throw new functions.https.HttpsError('permission-denied', 'Not a participant');

        const winnerUid = isCreator ? match.acceptor?.uid : match.creator?.uid;
        if (!winnerUid) throw new functions.https.HttpsError('failed-precondition', 'Opponent not found');

        const role = isCreator ? 'creator' : 'acceptor';
        const commissionPercent = await getLudoCommission();
        const poolAmount = match.amount * 2;
        const commission = Math.ceil(poolAmount * (commissionPercent / 100));
        const winAmount = poolAmount - commission;

        // Credit winner's winning balance
        await db.ref(`users/${winnerUid}/winningBalance`).transaction(b => (b || 0) + winAmount);
    ...
    if (resultType === 'WON') {
        ...
        if (opponentResult?.type === 'LOST') {
            ...
            await db.ref(`users/${uid}/winningBalance`).transaction(b => (b || 0) + winAmount);
    ...
    if (resultType === 'DISPUTE') {
        ...
        if (!match) throw new functions.https.HttpsError('not-found', 'Match not found');
        if (!['ROOM_SHARED', 'IN_PROGRESS'].includes(match.status)) {
            throw new functions.https.HttpsError('failed-precondition', 'Cannot dispute in current state');
        }
...
exports.cancelLudoMatch = functions.https.onCall(async (data, context) => {
    const uid = assertAuth(context);
    const { matchId, cancelType } = data;
    ...
    if (cancelType === 'MUTUAL_REQUEST') {
        ...
        if (match.status !== 'ROOM_SHARED' && match.status !== 'IN_PROGRESS') {
            throw new functions.https.HttpsError('failed-precondition', 'Mutual cancel only for active matches');
        }

        const isCreator = match.creator?.uid === uid;
        if (match.cancelRequest?.requestedBy === uid) {
            throw new functions.https.HttpsError('already-exists', 'Already requested');
        }
    ...
    throw new functions.https.HttpsError('invalid-argument', 'Invalid cancelType');
});
...
// ─── 3. Create Ludo Challenge ───────────────────────────────

exports.createLudoChallenge = functions.https.onCall(async (data, context) => {
    const uid = assertAuth(context);
    const { amount, ludoKingUsername } = data;
    ...
    if (!ludoKingUsername || ludoKingUsername.length < 2) {
        throw new functions.https.HttpsError('invalid-argument', 'Ludo King username required');
    }

    // ── Anti-Spam: Fetch all configs in parallel ──
    const [minSnap, maxSnap, maxOpenSnap, maxActiveSnap, cooldownSnap, userSnap] = await Promise.all([
    ...
    if (amount < minChallenge) {
        throw new functions.https.HttpsError('invalid-argument', `Minimum challenge is ₹${minChallenge}`);
    }

    // ── Check 2: Max amount cap ──
    if (amount > maxChallenge) {
        throw new functions.https.HttpsError('invalid-argument', `Maximum challenge is ₹${maxChallenge}`);
    }
    ...
    if (now - lastCreated < cooldownMs) {
        ...
        throw new functions.https.HttpsError('resource-exhausted', `Please wait ${waitSec}s before creating another challenge`);
    }
...
exports.acceptLudoChallenge = functions.https.onCall(async (data, context) => {
    const uid = assertAuth(context);
    const { matchId, ludoKingUsername } = data;
    ...
    if (!matchTxn.committed) {
        throw new functions.https.HttpsError('failed-precondition', 'Challenge no longer available');
    }
    ...
    if (!deduction) {
        ...
        throw new functions.https.HttpsError('failed-precondition', `Insufficient balance. Need ₹${matchAmount}`);
    }
