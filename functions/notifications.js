 ...
 *          sendEsportsPrizePush, sendEsportsRoomCredsPush, sendTournamentLobbyCredsPush
 */
...
// ─── Tournament Lobby Room Credentials Push ─────────────────

exports.sendTournamentLobbyCredsPush = onCall(async (request) => {
    await assertAdmin(request);

    const { tournamentId, roundKey, lobbyKey, roomId, roomPassword, lobbyName } = request.data;
    if (!tournamentId || !roundKey || !lobbyKey) {
        throw new HttpsError('invalid-argument', 'tournamentId, roundKey, and lobbyKey required');
    }
    if (!roomId || !roomPassword) {
        throw new HttpsError('invalid-argument', 'roomId and roomPassword required');
    }

    const tournSnap = await db.ref(`tournaments/${tournamentId}`).once('value');
    const tournament = tournSnap.val();
    if (!tournament) throw new HttpsError('not-found', 'Tournament not found');

    const lobby = tournament.lobbies?.[roundKey]?.[lobbyKey];
    if (!lobby) throw new HttpsError('not-found', 'Lobby not found');

    const players = lobby.players || {};
    // Extract real UIDs — for team modes, keys are team_X_Y, so use bookedBy
    const uids = [...new Set(
        Object.entries(players)
            .map(([key, data]) => data.bookedBy || key)
            .filter(uid => uid && uid.length > 15 && !uid.startsWith('team_'))
    )];

    if (uids.length === 0) {
        return { success: true, message: 'No players in lobby', notified: 0 };
    }

    const tournTitle = tournament.title || 'Tournament';
    const displayName = lobbyName || lobbyKey;
    const round = tournament.currentRound || 1;

    // Save to user_notifications for in-app display
    const notifId = db.ref('admin_notifications').push().key;
    const notificationData = {
        id: notifId,
        title: `🔑 Room Details — ${tournTitle}`,
        body: `Room ID: ${roomId} | Password: ${roomPassword}\nRound ${round} • ${displayName}. Join before the match starts!`,
        type: 'TOURNAMENT_ROOM_CREDS',
        tournamentId,
        sender: 'system',
        sentAt: new Date().toISOString(),
        timestamp: admin.database.ServerValue.TIMESTAMP
    ...
    // Send FCM push to each player
    let sent = 0;
    const promises = uids.map(async (uid) => {
        try {
            await sendPush(uid,
                `🔑 Room Details — ${tournTitle} (Round ${round})`,
                `Room ID: ${roomId} | Password: ${roomPassword}\n${displayName} — Join before the match starts!`,
                { type: 'TOURNAMENT_ROOM_CREDS', tournamentId, roomId, roomPassword }
            );
            sent++;
        } catch (err) {
            functions.logger.error(`Lobby creds push failed for ${uid}:`, err.message);
        }
    });

    await Promise.all(promises);

    functions.logger.info(`[sendTournamentLobbyCredsPush] ${tournTitle} ${displayName}: ${sent}/${uids.length} pushes delivered`);

    return { success: true, notified: sent, totalPlayers: uids.length };
});
