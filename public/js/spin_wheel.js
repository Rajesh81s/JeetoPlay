...
const SW_DEFAULT_SEGMENTS = [
    ...
    { label: '🪙 10', color: '#ec4899', textColor: '#fff' },
    { label: '🪙 25 🎰', color: '#ef4444', textColor: '#fff' }
];
...
function renderWheel(configSegments) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        // Segment path
        svg += `<path d="M150,150 L${x1},${y1} A140,140 0 ${largeArc},1 ${x2},${y2} Z" fill="${color}" stroke="rgba(255,255,255,0.1)" stroke-width="1"/>`;
...
window.spinTheWheel = async function () {
    ...
    try {
        ...
        // Wheel spins clockwise; segment 0 is at top
        // Target angle = point to winning segment center
        const targetSegAngle = data.winIndex * segAngle + segAngle / 2;
        // Final rotation: multiple full spins + land on winning segment
        const fullSpins = 5 + Math.floor(Math.random() * 3); // 5-7 full rotations
        const finalRotation = swCurrentRotation + fullSpins * 360 + (360 - targetSegAngle);
