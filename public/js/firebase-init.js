...
// ═══ CORE FIREBASE (Critical — these must succeed) ═══
try {
    ...
    console.error("Firebase Core Init Error:", e);
}

// ═══ SERVICE WORKER (Independent — must always register) ═══
...
// ═══ DEFERRED SDKS (Storage + Messaging — loaded after defer scripts) ═══
// These SDKs are defer-loaded so they won't be available immediately
window.addEventListener('load', () => {
    try {
        ...
        console.warn('Storage init deferred:', e.message);
    }

    try {
        if ('PushManager' in window && typeof firebase.messaging === 'function') {
            messaging = firebase.messaging();
            console.log('✅ Firebase Messaging initialized');
        }
    } catch (e) {
        console.warn('Messaging init deferred:', e.message);
    }
});

// Lazy getter for storage — returns null if SDK not yet loaded
Object.defineProperty(window, 'fbStorage', {
    get: function () {
        if (!storage && typeof firebase.storage === 'function') {
