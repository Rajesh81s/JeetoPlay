...
window.loadSmartNotifications = async function () {
    ...
    try {
        ...
        const [dailyRewardSnap, spinSnap, vipSnap, vipConfigSnap, balanceSnap] = await Promise.all([
            db.ref(`users/${uid}/dailyReward`).once('value'),
            db.ref(`users/${uid}/spinData`).once('value'),
            db.ref(`users/${uid}/vip`).once('value'),
            db.ref('platform_config/vip').once('value'),
            db.ref(`users/${uid}/depositBalance`).once('value')
        ]);

        const dailyReward = dailyRewardSnap.val() || {};
        const spinData = spinSnap.val() || {};
        const vipData = vipSnap.val() || {};
        const vipConfig = vipConfigSnap.val() || {};
        const depositBalance = balanceSnap.val() || 0;
